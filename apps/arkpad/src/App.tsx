...
export function App() {
  ...
  const editor = useArkpadEditor({
    content: "<p>Welcome to <strong>Arkpad</strong> — now with the simplest API ever.</p><p>We have introduced the <code>useArkpadEditor</code> hook to make your life easy.</p>",
  });
    ...
    <div className="min-h-screen bg-[#f8f9fc] py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col gap-8">
      <header className="flex flex-col items-center text-center gap-3">
        ...
        <h1 className="text-5xl font-black tracking-tight text-slate-900 sm:text-6xl">
          Ark<span className="text-blue-600">pad</span> <span className="font-light text-slate-400">Easy</span>
        </h1>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-[0_20px_80px_rgba(0,0,0,0.06)] border border-slate-200/60 overflow-hidden flex flex-col">
        ...
        <div className="flex flex-wrap items-center gap-1.5 p-3 bg-slate-50/80 border-b border-slate-200/60 sticky top-0 z-30 backdrop-blur-sm">
          <div className="flex items-center gap-1 bg-white p-1 rounded-2xl shadow-sm border border-slate-200/40">
            <ToolbarButton onClick={() => run("toggleBold")} isActive={isActive("strong")} title="Bold"><Bold className="w-4 h-4" /></ToolbarButton>
            <ToolbarButton onClick={() => run("toggleItalic")} isActive={isActive("em")} title="Italic"><Italic className="w-4 h-4" /></ToolbarButton>
          ...
          <div className="flex items-center gap-1 bg-white p-1 rounded-2xl shadow-sm border border-slate-200/40">
            <ToolbarButton onClick={() => run("toggleHeading", { level: 1 })} isActive={isActive("heading", { level: 1 })} title="H1"><Heading1 className="w-4 h-4" /></ToolbarButton>
            <ToolbarButton onClick={() => run("toggleHeading", { level: 2 })} isActive={isActive("heading", { level: 2 })} title="H2"><Heading2 className="w-4 h-4" /></ToolbarButton>
            <ToolbarButton onClick={() => run("toggleHeading", { level: 3 })} isActive={isActive("heading", { level: 3 })} title="H3"><Heading3 className="w-4 h-4" /></ToolbarButton>
          </div>

          <ToolbarSeparator />

          <div className="flex items-center gap-1 bg-white p-1 rounded-2xl shadow-sm border border-slate-200/40">
            <ToolbarButton onClick={() => run("toggleBlockquote")} isActive={isActive("blockquote")} title="Blockquote"><Quote className="w-4 h-4" /></ToolbarButton>
            <ToolbarButton onClick={() => run("toggleCodeBlock")} isActive={isActive("codeBlock")} title="Code Block"><Terminal className="w-4 h-4" /></ToolbarButton>
            <ToolbarButton onClick={() => run("setHorizontalRule")} title="Horizontal Rule"><Minus className="w-4 h-4" /></ToolbarButton>
            <ToolbarButton onClick={() => run("setImage", "https://picsum.photos/1200/600")} title="Insert Image"><ImageIcon className="w-4 h-4" /></ToolbarButton>
          </div>

          <ToolbarSeparator />

          <div className="flex items-center gap-1 bg-white p-1 rounded-2xl shadow-sm border border-slate-200/40">
            <ToolbarButton onClick={() => run("toggleBulletList")} isActive={isActive("bullet_list")} title="Bullet List"><List className="w-4 h-4" /></ToolbarButton>
            <ToolbarButton onClick={() => run("toggleOrderedList")} isActive={isActive("ordered_list")} title="Ordered List"><ListOrdered className="w-4 h-4" /></ToolbarButton>
            <ToolbarButton onClick={() => run("toggleTaskList")} isActive={isActive("task_list")} title="Task List"><CheckSquare className="w-4 h-4" /></ToolbarButton>
          </div>

          <div className="flex-1 min-w-4" />

          <div className="flex items-center gap-1 bg-white p-1 rounded-2xl shadow-sm border border-slate-200/40">
            <ToolbarButton onClick={() => run("undo")} disabled={!canUndo} title="Undo"><Undo2 className="w-4 h-4" /></ToolbarButton>
            <ToolbarButton onClick={() => run("redo")} disabled={!canRedo} title="Redo"><Redo2 className="w-4 h-4" /></ToolbarButton>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between px-8 py-3 bg-white rounded-full shadow-sm border border-slate-200/60 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" /> Hook Enabled</span>
          <span>&bull;</span>
