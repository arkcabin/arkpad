...
import { isMarkActive, isNodeActive, getMarkAttributes, getNodeAttributes, } from "./extensions/utils";
...
export class ArkpadEditor {
    ...
    refreshState(content = this.view.state.doc.toJSON()) {
        const { schema } = this.extensionManager;
     ...
     */
    setContent(content, format, emitUpdate = true) {
        const { schema } = this.extensionManager;
     ...
     */
    focus(pos) {
        if (this.view.hasFocus() && pos === undefined)
            return;
        this.view.focus();
        if (pos === "start") {
            this.setSelection(0);
    ...
    createCommandsProxy() {
        return new Proxy({}, {
            get: (_, prop) => {
                const command = this.extensionManager.commands[prop];
                if (command) {
                    return (...args) => this.runCommand(prop, ...args);
                }
 ...
 */
export function createArkpadEditor(options) {
